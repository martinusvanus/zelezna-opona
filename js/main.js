...
function onEachFeature(feature, layer) {
    layer.on('click', function () {
        //formátuje popupy
        document.getElementById("popupcontent").innerHTML = "<dl><dt>Jméno osoby</dt><dd>" + feature.properties.vlastni_jmeno + " " + feature.properties.prijmeni_ini + "</dd><dt>Státní příslušnost</dt><dd>" + feature.properties.statni_prislusnost + "</dd><dt>Rok incidentu</dt><dd>" + feature.properties.rok + "</dd><dt>Odpovědný útvar</dt><dd>" + feature.properties.utvar_incidentu + "</dd></dl><dl style='margin-left: 15px;'><dt>Věk</dt><dd>" + feature.properties.vek_h + "</dd><dt>Okres bydliště</dt><dd>" + feature.properties.okres_bydliste_cz + "</dd><dt>Směr přechodu</dt><dd>" + feature.properties.smer_prechodu + "</dd><dt>Způsob úmrtí</dt><dd>" + feature.properties.umrti + "</dd></dl>";
        $("#popup").stop();
        $("#popup").fadeIn("fast");
...
var filterObeti = function (){
    ...
    //upraví počet obětí
    if (obeti.length == 0 || obeti.length >= 5) {
        document.getElementById("pocet-obeti").innerHTML = obeti.length + " usmrcených";
    } if (obeti.length == 1) {
        document.getElementById("pocet-obeti").innerHTML = obeti.length + " usmrcený";
    } if (obeti.length >= 2 && obeti.length <= 4) {
        document.getElementById("pocet-obeti").innerHTML = obeti.length + " usmrcení";
    }
